...
void StringTableGIF::Initialize(int minCodeSize)
{
	m_done = false;
...
BYTE *StringTableGIF::FillInputBuffer(int len)
{
	if( m_buffer == NULL ) {
...
void StringTableGIF::CompressStart(int bpp, int width)
{
	m_bpp = bpp;
...
int StringTableGIF::CompressEnd(BYTE *buf)
{
	int len = 0;
...
bool StringTableGIF::Compress(BYTE *buf, int *len)
{
...
bool StringTableGIF::Decompress(BYTE *buf, int *len)
{
	if( m_bufferSize == 0 || m_done ) {
		return false;
	}
...
void StringTableGIF::Done(void)
{
	m_done = true;
}

void StringTableGIF::ClearCompressorTable(void)
{
	if(m_strmap) {
...
void StringTableGIF::ClearDecompressorTable(void)
{
...
Load(FreeImageIO *io, fi_handle handle, int page, int flags, void *data) {
	...
	try {
		...
		StringTableGIF *stringtable = new(std::nothrow) StringTableGIF;
